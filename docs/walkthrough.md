...
We can start up the debug infrastructure. On a new terminal session run `fuel-core --db-type in-memory --debug`; we need to have that running because it actually executes the program. Now we can fire up the debugger itself: `fuel-debugger`. Now
